...
class QtAppServices:
    ...
    def _recording_path_under_root(self, recording: RecordingSummary, *, reject_symlink: bool = False) -> Path:
        raw_path = Path(recording.path)
        if reject_symlink and raw_path.is_symlink():
            raise ValueError("Cannot delete symlinked recording folders.")
        recording_path = raw_path.resolve()
    ...
    def rename_recording(self, recording: RecordingSummary, title: str) -> None:
        if is_recording_active(self.recordings_root):
            raise RuntimeError("Stop the active recording before renaming recordings.")
    ...
    def delete_recording(self, recording: RecordingSummary) -> None:
        recording_path = self._recording_path_under_root(recording, reject_symlink=True)
        if is_recording_active(self.recordings_root):
